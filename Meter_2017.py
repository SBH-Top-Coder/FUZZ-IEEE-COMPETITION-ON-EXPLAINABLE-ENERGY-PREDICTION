...
class Meter_2017 :
  def __init__(self ,df) :
    self.meter_id = df.iloc[0].values[0]
    self.meter = df[1:]
  def drop_duplicate (self) :
    self.meter.reset_index(level=0, inplace=True)
    self.meter.columns = ['Datetime','Consommation']
    self.meter.drop_duplicates(subset='Datetime', keep='last', inplace=True)
    date_range = pd.date_range(start='2017-01-01',
                           end='2018-01-01',
                           freq='H')
    print(f'The difference in length between the custom date range and our dataset is {(len(date_range)*2-len(self.meter)-2)}:')
    # conclusion : puisque c'est 0 donc  les index kol moujoudin
  def time_features (self) :
    ...
    self.meter = df_train
    self.meter.columns = ["Datetime" ,"consommation" ,"year","month", "day", "hour","minute"]
    ...
    self.meter['day_string'] = [pd.to_datetime(self.meter['Datetime'][i]).strftime("%A") for i in range(len(self.meter)) ]
  ...
  def casting(self) :
    ...
    self.meter['consommation'] = self.meter['consommation'].astype('float')
  def mapping (self) :
    ...
    self.meter['dwelling_type'] = self.meter['dwelling_type'].map({'_':-1 ,'semi_detached_house':1,'detached_house':2,
